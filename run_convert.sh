...
MODEL_PATH="/mnt/dhwfile/raise/user/linhonglin/vlm/models/qwen3_tikz_sft_convert_debug"
INPUT_JSONL="/mnt/dhwfile/raise/user/linhonglin/vlm/stages/text_data/AM-Thinking-v1-Distilled-math.jsonl"
OUTPUT_JSONL="/mnt/dhwfile/raise/user/linhonglin/vlm/stages/text_data/AM-Thinking-v1-Distilled-math_output_0.3.jsonl"
PROMPT_NAME="convert"
...
if [[ -z "$MODEL_PATH" || -z "$INPUT_JSONL" || -z "$OUTPUT_JSONL" || -z "$PROMPT_NAME" || -z "$MODEL_NAME" ]]; then
  echo "❌ 缺少必要参数：--model_path --input_jsonl --output_jsonl --prompt_name"
  exit 1
fi
...
# 5️⃣ 运行主程序并记录日志
srun -p belt_road --gres=gpu:1 python main.py \
